...
export async function GET(request) {
  try {
    ...
    const params = []
    let query = 'SELECT p.*, c.company_name FROM main.pocs p LEFT JOIN main.companies c ON p.company_id = c.id'
...
export async function POST(request) {
  try {
    ...
    const result = await pool.query(`
      INSERT INTO main.pocs (company_id, email, name, phone, role, status, is_primary, created_at, updated_at)
