...
export default function ZiwoWidget({ contactCenterName = 'iohealth' }) {
  ...
  // Cortex agent status (available / break / not_ready / meeting / busy / offline)
  const [cortexStatus, setCortexStatus] = useState('available')
  ...
  const disconnectTimerRef = useRef(null) // debounce disconnect → ERROR
  ...
  // Fetch call dispositions once on mount
  useEffect(() => {
    fetch('/api/admin/dispositions')
      .then(r => r.ok ? r.json() : [])
      .then(d => setDispositions(Array.isArray(d) ? d : []))
      .catch(() => {})
  }, [])
  ...
  // Fetch actual DB status immediately on mount so UI shows real status before ZIWO connects
  useEffect(() => {
    if (!session?.user?.id) return
    fetch(`/api/users/${session.user.id}/status`)
      .then(r => r.ok ? r.json() : null)
      .then(data => { if (data?.status) setCortexStatus(data.status) })
      .catch(() => {})
  }, [session?.user?.id])
  ...
  useEffect(() => {
    ...
    function onConnected(e) {
      // Cancel any pending disconnect → ERROR transition
      ...
      // Read the agent's actual current status from DB instead of forcing 'available'
      if (session?.user?.id) {
        fetch(`/api/users/${session.user.id}/status`)
          .then(r => r.ok ? r.json() : null)
          .then(data => {
            const currentStatus = data?.status || 'available'
            setCortexStatus(currentStatus)
            window.dispatchEvent(new CustomEvent('cortex-ziwo-connected', { detail: { status: currentStatus } }))
          })
          .catch(() => {
    ...
    function onDisconnected() {
      // Debounce: wait 5s before showing error — SDK often reconnects automatically
    ...
    function onHangup(e) {
      ...
      const cause = e.detail?.origin || e.detail?.cause || 'normal'
      const direction = e.detail?.direction || callInfo?.direction || '—'
      const ended = {
        duration: secs,
        direction,
      ...
      // Missed/unanswered inbound: just go back to connected, no post-call form
      const isMissed = secs === 0 || cause === 'missed' || cause === 'caller_cancel' || cause === 'originator_cancel'
    ...
    function onDestroy() {
      if (currentCallRef.current) {
        stopTimer()
        currentCallRef.current = null
        // Only reset to CONNECTED from ACTIVE state — don't interrupt RINGING
        setStatus(prev => prev === S.ACTIVE ? S.CONNECTED : prev)
      }
    }
  ...
  // Sync Cortex status changes to ZIWO agent state + track local cortexStatus
  useEffect(() => {
    function onStatusChange(e) {
      const s = e.detail?.status
      if (s) setCortexStatus(s)
  ...
  const statusLabel = {
    ...
    [S.CONNECTED]: agentName ? `${agentName} — ${cortexStatusLabel}` : `Connected — ${cortexStatusLabel}`,
  ...
  const canDial = status === S.CONNECTED && cortexStatus === 'available'
  ...
  return (
    <>
      ...
      >
        ...
        {/* Compact ringing strip — visible even when minimized */}
        {minimized && status === S.RINGING && (
          <div className="px-3 py-2 bg-cortex-warning/10 border-t border-cortex-warning/20 flex items-center gap-2">
            <span className="text-xs text-cortex-warning font-mono animate-pulse flex-1 truncate">
              📞 {callInfo?.number || 'Unknown'}
            </span>
            <button
              onClick={handleAnswer}
              className="px-2.5 py-1 rounded-lg bg-cortex-success/20 text-cortex-success border border-cortex-success/30 text-xs font-semibold hover:bg-cortex-success/30 transition-colors"
            >
              Answer
            </button>
            <button
              onClick={handleReject}
              className="px-2.5 py-1 rounded-lg bg-cortex-danger/20 text-cortex-danger border border-cortex-danger/30 text-xs font-semibold hover:bg-cortex-danger/30 transition-colors"
            >
              Reject
            </button>
          </div>
          ...
          <div className="p-4 space-y-3">
            ...
            {status === S.ENDED && endedInfo && (
              <div className="space-y-3">
                ...
                {/* Disposition picker (Item 24) */}
                {dispositions.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-cortex-text mb-1.5">Call Outcome</p>
                    <div className="flex flex-wrap gap-1.5">
                      {dispositions.map(d => (
                        <button
                          key={d.id}
                          onClick={() => setSelectedDisposition(prev => prev === d.id ? null : d.id)}
                          className={`text-xs px-2 py-1 rounded-lg border transition-colors ${
                            selectedDisposition === d.id
                              ? 'bg-cortex-accent/20 text-cortex-accent border-cortex-accent/40'
                ...
                {showTicketForm ? (
                  <div className="space-y-2.5 border-t border-cortex-border/60 pt-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => {
                          if (selectedDisposition && endedInfo?.primaryCallId) {
                            updateCallLog(endedInfo.primaryCallId, { disposition_id: selectedDisposition }).catch(() => {})
                          }
                          setShowTicketForm(false)
                          setEndedInfo(null)
                          setIsFCR(false)
            ...
            {status === S.CONNECTED && (
              <div className="space-y-2">
                ...
                {!canDial && (
                  <div className="rounded-lg bg-cortex-warning/10 border border-cortex-warning/30 px-3 py-2 text-xs text-cortex-warning">
                    Status is <span className="font-semibold">{cortexStatusLabel}</span> — change to Available to make calls
                  </div>
                )}

                {/* Number display + backspace */}
                <div className={`flex gap-2 ${!canDial ? 'opacity-40 pointer-events-none' : ''}`}>
                ...
                {/* Numeric keypad */}
                <div className={`grid grid-cols-3 gap-1.5 ${!canDial ? 'opacity-40 pointer-events-none' : ''}`}>
                  {DIAL_KEYS.flat().map(key => (
                    <button
                      key={key}
                      onClick={() => handleDialKey(key)}
                      disabled={!canDial}
                      className="py-2.5 rounded-lg bg-cortex-bg border border-cortex-border text-cortex-text text-sm font-mono font-medium hover:bg-cortex-border hover:border-cortex-muted transition-colors active:scale-95 disabled:cursor-not-allowed"
                    >
                      {key}
                    </button>
                  ))}
        ...
        {/* Minimized incoming call — answer/reject (Item 27) */}
        {minimized && status === S.RINGING && (
          <div className="flex gap-1.5 px-3 pt-0.5 pb-2.5">
            <button
              onClick={handleAnswer}
              onMouseDown={e => e.stopPropagation()}
              className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg bg-cortex-success/20 text-cortex-success border border-cortex-success/30 hover:bg-cortex-success/30 text-xs font-medium transition-colors"
            >
              <PhoneIncoming className="w-3 h-3" /> Answer
            </button>
            <button
              onClick={handleReject}
              onMouseDown={e => e.stopPropagation()}
              className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg bg-cortex-danger/20 text-cortex-danger border border-cortex-danger/30 hover:bg-cortex-danger/30 text-xs font-medium transition-colors"
            >
              <PhoneOff className="w-3 h-3" /> Reject
            </button>
          </div>
        )}
      </div>
