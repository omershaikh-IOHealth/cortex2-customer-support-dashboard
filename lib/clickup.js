...
/**
 * Map Cortex status names to ClickUp status names.
 * ClickUp statuses: OPEN, IN PROGRESS, ON HOLD, COMPLETE
 */
function mapStatus(cortexStatus) {
  ...
  return map[cortexStatus?.toLowerCase()] ?? cortexStatus?.toLowerCase()
}
...
export async function addClickUpComment(clickupTaskId, commentText) {
  ...
  try {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}))
      console.error('[clickup] addComment failed:', res.status, err)
      return false
    }
 ...
 * Returns flat array of { id, email, username } or [] on failure.
 */
export async function getClickUpMembers() {
  ...
  try {
    ...
    if (!res.ok) {
      console.error('[clickup] getMembers failed:', res.status)
      return []
    }
    ...
    console.error('[clickup] getMembers error:', err)
    return []
  }
}
...
export async function updateClickUpTask(clickupTaskId, changes = {}) {
  ...
  try {
    ...
    if (changes.assignees)   body.assignees = changes.assignees
