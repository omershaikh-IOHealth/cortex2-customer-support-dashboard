export const getSchemaContext = (userRole, userCompanyId, userEmail) => {
    const roleFilter = userRole === 'agent'
      ? `ROLE: agent (${userEmail || 'unknown'})
DATA ACCESS RULES — strictly enforced:
  ALLOWED:
    - Tickets WHERE assigned_to_email = '${userEmail}' (this agent's own tickets only)
    - Aggregate/count queries across all tickets (e.g. "how many P1 tickets are open?") — these are org-wide stats and are allowed
    - SLA rates, escalation counts, priority distributions — aggregate stats are fine
    - This agent's own call_logs, shift_rotas, shift_breaks, notifications
    - All circulars (knowledge base) — read only
  NOT ALLOWED (refuse or return nothing):
    - Individual tickets NOT assigned to '${userEmail}'
    - Other agents' call logs or status
    - users table (password hashes, credentials, auth data)
    - sla_configs, escalation_configs, assignee_configs, kpi_configs (admin config)
    - auth_logs
  Always also filter: is_deleted = FALSE`
      : `ROLE: admin — Full read access to all tables. No additional restrictions.`;
  ...
  JOINS:
  - Tickets → Companies: JOIN main.companies c ON t.company_id = c.id
  - Tickets → Solutions: JOIN main.solutions s ON t.solution_id = s.id
  - Tickets → POCs: JOIN main.pocs p ON t.poc_id = p.id
  - Threads → Tickets: JOIN main.tickets t ON th.ticket_id = t.id
  - Call logs → Agent: JOIN main.users u ON cl.agent_id = u.id
  - Call logs → Ticket: JOIN main.tickets t ON cl.ticket_id = t.id
  - Agent status → User: JOIN main.users u ON ast.user_id = u.id
  - Shifts → User: JOIN main.users u ON sr.user_id = u.id
  - Notifications → User: JOIN main.users u ON n.user_id = u.id
  - Circulars → Creator: JOIN main.users u ON cir.created_by = u.id
  
  IMPORTANT:
  - Always exclude deleted tickets: is_deleted=FALSE
  - Use schema prefix: main.tickets, main.companies, etc.
